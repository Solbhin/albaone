	                        	...
	                        	<c:if test="${attendance.date==day.date}">
									...
									<c:choose>
										<c:when test="${attendance.checkOutTime==null}">
				                            ...
				                            <c:if test="${not empty sessionScope.businessNumber}">
					                            <a href="attendanceEdit?id=${attendance.id}&checkInTime=${attendance.checkInTime}" class="btn btn-warning">보기</a></p>
					                        </c:if> 
										</c:when>
			                            <c:otherwise>
			                            	${attendance.time}
			                            	<c:if test="${not empty sessionScope.businessNumber}">
			                            		<a href="attendanceEdit?id=${attendance.id}&checkInTime=${attendance.checkInTime}&checkOutTime=${attendance.checkOutTime}" class="btn btn-warning">보기</a></p>
			                            	</c:if>
			                            </c:otherwise>
									</c:choose>
		                        </c:if>
